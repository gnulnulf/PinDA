...
void MC6821::write_cra( uint8_t data ) {
	cra_sv=data;
	bus->write( cra ,data);
} //write_cra

void MC6821::write_crb( uint8_t data ) {
	crb_sv=data;
	bus->write( crb ,data);
}

void MC6821::write_ddra( uint8_t data){
	cra_sv &= ~(1<<DDRA_XS);
	bus->write( cra ,cra_sv);
	ddra_sv=data;
	bus->write( ddra ,ddra_sv);
}
void MC6821::write_ddrb(uint8_t data){
	crb_sv &= ~(1<<DDRB_XS);
...
void MC6821::write_pdra(uint8_t data){
	cra_sv |= (1<<DDRA_XS);
	bus->write( cra ,cra_sv);
	pdra_sv=data;
	bus->write( pdra ,pdra_sv);
}
void MC6821::write_pdrb(uint8_t data){
	crb_sv |= (1<<DDRB_XS);
	bus->write( crb ,crb_sv);
	pdrb_sv=data;
...
uint8_t MC6821::read( int address ){
	return 0;
}

void MC6821::write (int address, uint8_t data){
