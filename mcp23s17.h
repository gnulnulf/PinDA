...
//	MCP23X17(void) =0;
	// 16 bit commands
virtual    void pinMode(bool mode);
    virtual void pinMode(uint8_t pin, bool mode);
    virtual void port(uint16_t value);
    virtual uint16_t port();

	// Work on individual pins 0-15
    virtual void digitalWrite(uint8_t pin, bool value);
    virtual int  digitalRead(uint8_t pin);

	
	// 8 bit port A commands
	virtual void pinModeA( uint8_t pin, bool state );	
	virtual void pinA( uint8_t pin, bool state );
	virtual uint8_t portA();
    virtual void portA(uint8_t value);
	virtual void ddrA(uint8_t val);

	// 8 bit port B commands
	virtual void pinB( uint8_t pin, bool state );
	virtual uint8_t portB();
    virtual void portB(uint8_t value);
	virtual void ddrB(uint8_t val);
	virtual void pinModeB( uint8_t pin, bool state );
...
//const static uint8_t UNUSED = 0b00000001;
	...
	virtual uint16_t read_addr(uint8_t addr)=0;
    virtual uint8_t read_addr_byte(uint8_t addr)=0;
    virtual void write_addr(uint8_t addr, uint16_t data)=0;
	virtual	void write_addr_byte(uint8_t addr, uint8_t data)=0;

	virtual uint16_t byte2uint16(uint8_t high_byte, uint8_t low_byte);
	virtual uint8_t uint16_high_byte(uint16_t uint16);
	virtual uint8_t uint16_low_byte(uint16_t uint16);
